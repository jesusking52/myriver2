...
public class HighlightView {
    ...
    private double maxZoomFactor = 0;
    ...
    private void init() {
        ...
        mResizeDrawableWidth = resources.getDrawable(R.drawable.btn_common_crop_width);
        mResizeDrawableHeight = resources.getDrawable(R.drawable.btn_common_crop_height);
        mResizeDrawableDiagonal = resources.getDrawable(R.drawable.indicator_autocrop);
    }

    public boolean mIsFocused;
    boolean mHidden;
    public boolean mThirds;

    public void setThirds(boolean thirds){
        mThirds = thirds;
    }
    ...
    public void draw(Canvas canvas) {
        ...
        if (!hasFocus()) {
            ...
            if (mCircle) {
                ...
                int color = mContext.getResources().getColor(R.color.river_auction_dodger_blue);
            ...
            if (mThirds) {
                canvas.drawLine(mDrawRect.left + xThird, mDrawRect.top, mDrawRect.left + xThird, mDrawRect.bottom, mOutlinePaint);
                canvas.drawLine(mDrawRect.left + xThird * 2, mDrawRect.top, mDrawRect.left + xThird * 2, mDrawRect.bottom, mOutlinePaint);
                canvas.drawLine(mDrawRect.left, mDrawRect.top + yThird, mDrawRect.right, mDrawRect.top + yThird, mOutlinePaint);
                canvas.drawLine(mDrawRect.left, mDrawRect.top + yThird * 2, mDrawRect.right, mDrawRect.top + yThird * 2, mOutlinePaint);
            }
    ...
    void growBy(float dx, float dy) {
        ...
        if (getMaxZoomFactor() > 0 && (((mImageRect.width() / (r.width() + dx * 2)) > getMaxZoomFactor()) || ((r.width() + dx * 2) < 0))) {
            float adjustment = ((float) ((mImageRect.width() / getMaxZoomFactor()) - (r.width()))) / 2F;
            dx = adjustment;
            if (mMaintainAspectRatio) {
                dy = dx / mInitialAspectRatio;
            }
    ...
    /**
     * Crop rectangle position in screen space
     */
    public Rect mDrawRect;

    /**
     * Source image size (in image space, not changed after zoom in)
     */
    private RectF mImageRect;

    /**
     * Crop rectangle in image space
     */
