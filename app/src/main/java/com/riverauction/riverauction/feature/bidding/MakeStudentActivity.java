...
public class MakeStudentActivity extends BaseActivity implements MakeBiddingMvpView {
    private static final int REQUEST_PATCH_STUDENT_LESSON_INFO = 0x01;
    public static final String EXTRA_PROFILE_ID = "gggg" ;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getSupportActionBar().setTitle("과외요청 학생 프로필");
        getSupportActionBar().setHomeButtonEnabled(true);
        getSupportActionBar().setDisplayHomeAsUpEnabled(true);

        makeBiddingButton.setOnClickListener(v ->{
            Intent intent2 = new Intent(Intent.ACTION_DIAL, Uri.parse("tel:"+user.getPhoneNumber()));
    ...
    private void getDataFromBundle(Bundle bundle) {
        if (bundle != null) {
            ...
            int ownerId = bundle.getInt(EXTRA_PROFILE_ID, -1);
            if (ownerId == -1) {
                throw new IllegalStateException("ownerId must be exist");
            }
            presenter.getUserProfile(ownerId, true);
        }
    }
    ...
    private void setContent(CUser user) {
        ...
        basicInfoView.setContent2(user);
